...
const restaurant: WebsiteSpec = {
  ...
  seo: { title: 'Premium Indian Restaurant | Modern Indian Dining', description: 'A luxury Indian restaurant serving contemporary cuisine in an elegant dining setting.' },
  ...
  hero: { eyebrow: 'Contemporary Indian dining', headline: 'Indian cuisine, beautifully reimagined.', body: 'A polished dining experience built around regional Indian flavours, seasonal ingredients and warm, attentive hospitality.', cta: 'Reserve your table', assetName: 'restaurant-hero.jpg' },
  sections: [
...
test('exact restaurant scope keeps basics inexpensive', () => {
  const state = { plan: 'Starter', pages: ['Home', 'About', 'Menu', 'Gallery', 'Contact'], features: ['WhatsApp button', 'Google Maps', 'Contact form', 'Testimonials', 'Gallery', 'SEO setup', 'Analytics-ready'] };
...
test('export policy remains locked before verified payment', () => {
  for (const status of ['DRAFT', 'CONFIGURED', 'GENERATING', 'PREVIEW_READY', 'PAYMENT_PENDING', 'PAID']) assert.equal(canExport(status), false, `${status} must remain locked`);
