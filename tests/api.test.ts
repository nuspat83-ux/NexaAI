...
test('Vercel-compatible API handler covers routing, auth, generation, payment config and export lock', async () => {
  ...
  try {
    ...
    try {
      ...
      globalThis.fetch = async (input, init) => {
        ...
        if (url.includes('generativelanguage.googleapis.com')) {
          ...
          const response = prompt.includes("website planning assistant")
            ? (/Customer request:.*\b(cafe|coffee|restaurant)\b/i.test(prompt) ? { category:'Cafe', businessName:'Harbour Cafe', description:'A neighbourhood cafe.', location:'Mumbai', phone:'', whatsapp:'', email:'', address:'', hours:'9am-9pm', socials:'', brief:'A cafe website with menu and ordering.', audience:'Local diners', styles:['Modern'], pages:['Home','Menu','Contact'], sections:['Hero','Menu','Contact'], features:['Google Maps','WhatsApp button'], catalogMode:'menu', orderFlow:'whatsapp', products:[{id:'m1',name:'Masala Chai',description:'Tea with spices',price:120,category:'Drinks'}], services:[], businessDetails:{openingHours:'9am-9pm'}, clarifyingQuestions:[] }
            : /Customer request:.*\b(ac repair|air conditioner|service business)\b/i.test(prompt) ? { category:'Local Business', businessName:'AC Care', description:'AC repair service.', location:'Mumbai', phone:'', whatsapp:'', email:'', address:'', hours:'', socials:'', brief:'AC repair service in Mumbai.', audience:'Homeowners', styles:['Professional'], pages:['Home','Services','Contact'], sections:['Hero','Services','Contact'], features:['WhatsApp button','Contact form'], catalogMode:'services', orderFlow:'none', products:[], services:[{id:'s1',name:'AC repair',description:'Diagnosis and repair'}], businessDetails:{serviceArea:'Mumbai'}, clarifyingQuestions:[] }
            : (prompt.includes('Additional clarification:') && !prompt.endsWith('none'))
              ? { category:'E-commerce', businessName:'Mumbai Threads', description:'A clothing shop selling curated apparel.', location:'Mumbai', phone:'', whatsapp:'+91 9000000000', email:'', address:'', hours:'', socials:'', brief:'My clothing shop is in Mumbai and I want a premium product website.', audience:'Fashion shoppers', styles:['Premium','Modern'], pages:['Home','Products','About','Contact'], sections:['Hero','Products','Testimonials','Contact'], features:['Product catalog','WhatsApp button','SEO setup'], catalogMode:'products', orderFlow:'whatsapp', products:[{id:'p1',name:'Black Dress',price:1299,size:'S, M, L',color:'Black',images:['dress.jpg']}], services:[], businessDetails:{products:'40 products',productCategories:'Clothing',payment:'Both online payment and WhatsApp ordering'}, clarifyingQuestions:[] }
      ...
      assert.equal(directBody.orderFlow, 'whatsapp');
      ...
      const cafePlan = await httpRequest(server, '/api/direct-plan', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ brief: 'A cafe in Mumbai with a menu, opening hours, WhatsApp ordering and Google Maps.' }),
      });
      assert.equal(cafePlan.status, 200);
      const cafeBody = await cafePlan.json() as Record<string, any>;
      assert.equal(cafeBody.category, 'Cafe');
      assert.equal(cafeBody.catalogMode, 'menu');
      assert.equal(cafeBody.products[0].name, 'Masala Chai');
      assert.equal(cafeBody.services.length, 0);

      const servicePlan = await httpRequest(server, '/api/direct-plan', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ brief: 'AC repair service in Mumbai.' }),
      });
      assert.equal(servicePlan.status, 200);
      const serviceBody = await servicePlan.json() as Record<string, any>;
      assert.equal(serviceBody.category, 'Local Business');
      assert.equal(serviceBody.catalogMode, 'services');
