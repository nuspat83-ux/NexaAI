...
export interface WebsiteSpec{business:string;tagline:string;seo?:{title:string;description:string};colors:{primary:string;secondary:string;background?:string;text?:string};typography?:{heading:string;body:string};personality?:string[];pages?:string[];sections:string[]|WebsiteSection[];features?:string[];catalogMode?:'products'|'menu'|'services'|'none';products?:import('./catalog').Product[];services?:import('./catalog').ServiceItem[];content:string;imageRequirements:string[];cta:string;responsive:string;nav?:string[];hero?:{eyebrow:string;headline:string;body:string;cta:string;assetName?:string};contact?:{phone:string;whatsapp:string;email:string;address:string;hours:string};footer?:string;assets?:Asset[];}
