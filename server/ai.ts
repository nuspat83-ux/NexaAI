...
const MAX_RETRIES=3,TIMEOUT_MS=90000;
const RETRY_DELAYS_MS=[1000,2000,4000];
function isTransientStatus(status:number){return status>=500&&status<=599;}
function retryDelay(attempt:number){const base=RETRY_DELAYS_MS[Math.min(attempt,RETRY_DELAYS_MS.length-1)];return base+Math.floor(Math.random()*250);}
async function waitForRetry(attempt:number){await new Promise(resolve=>setTimeout(resolve,retryDelay(attempt)));}
const SUPPORTED_GEMINI_MODELS = new Set(['gemini-3.5-flash','gemini-3.6-flash','gemini-3.7-flash','gemini-3.8-flash']);
function geminiModel(){const configured=process.env.GEMINI_MODEL?.trim();return configured&&SUPPORTED_GEMINI_MODELS.has(configured)?configured:'gemini-3.5-flash';}
...
async function requestGemini(s:BuilderState):Promise<WebsiteSpec>{const key=process.env.GEMINI_API_KEY;if(!key)throw new Error('GEMINI_API_KEY is not configured on the server');const parts:any[]=[{text:promptFor(s)}];for(const a of s.assets||[])if(a.data?.startsWith('data:image/'))parts.push({inlineData:{mimeType:a.type,data:a.data.replace(/^data:[^;]+;base64,/,'')}});const c=new AbortController(),timer=setTimeout(()=>c.abort(),TIMEOUT_MS);try{const r=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${geminiModel()}:generateContent`,{method:'POST',headers:{'Content-Type':'application/json','x-goog-api-key':key},signal:c.signal,body:JSON.stringify({contents:[{role:'user',parts}],generationConfig:{responseMimeType:'application/json'}})});if(isTransientStatus(r.status)){throw new RetryableAIError(`Gemini temporary error ${r.status}`);}if(r.status===429){const detail=await r.clone().json().catch(()=>null) as any;const code=String(detail?.error?.status||detail?.error?.code||'').toLowerCase();const message=String(detail?.error?.message||'').slice(0,300);if(['rate_limit_exceeded','too_many_requests'].includes(code))throw new RetryableAIError(`Gemini rate limit: ${message||'Please retry shortly.'}`);throw new Error(`Gemini quota limit reached: ${message||'Please check your Gemini API quota.'}`);}if(!r.ok)throw new Error(`Gemini request failed (${r.status})`);const p:any=await r.json(),text=p?.candidates?.[0]?.content?.parts?.map((z:any)=>z.text||'').join('');if(!text)throw new RetryableAIError('Gemini returned an empty response');const parsed=JSON.parse(extractJson(text));validate(parsed);const business=s.businessName.trim()||s.customBusiness.trim()||parsed.business;const contact={phone:s.phone,address:s.address,email:s.email,whatsapp:s.whatsapp,hours:s.hours};return {...parsed,business,tagline:s.tagline||parsed.tagline,features:s.features,catalogMode:s.catalog?.mode||'none',orderFlow:s.catalog?.orderFlow||'none',payment:s.catalog?.payment||'unknown',delivery:s.catalog?.delivery||{},products:s.catalog?.products||[],services:s.catalog?.services||[],contact,colors:{primary:parsed.colors?.primary||s.primary,secondary:parsed.colors?.secondary||s.secondary,background:parsed.colors?.background||'#f7f5f0',text:parsed.colors?.text||'#151515'},personality:s.styles.length?s.styles:['Modern','Premium'],imageRequirements:s.assets.map(a=>a.name),content:parsed.hero.body,cta:parsed.hero.cta,responsive:'Mobile-first with tablet and desktop adaptations',assets:s.assets} as WebsiteSpec;}finally{clearTimeout(timer);}}
export async function generateWebsite(s:BuilderState,onStage:(stage:string)=>void){let last:unknown;for(let attempt=0;attempt<=MAX_RETRIES;attempt++){try{onStage('Analyzing requirements');onStage('Planning website');onStage('Creating design system');onStage('Writing business content');onStage('Processing images');const result=await requestGemini(s);onStage('Building pages');onStage('Optimizing responsive layout');onStage('Running quality checks');onStage('Preparing preview');return result;}catch(e){last=e;const retryable=e instanceof RetryableAIError||(e instanceof Error&&/abort|timeout|empty response/i.test(e.message));if(!retryable||attempt===MAX_RETRIES)break;await waitForRetry(attempt);}}throw last instanceof Error?last:new Error('Website generation failed');}
...
export async function planDirectBrief(brief:string, clarification=''):Promise<DirectPlan>{
  ...
  try{
    ...
    for(let attempt=0;attempt<=MAX_RETRIES;attempt++){
      ...
      if(r.status===429){
        const detail=await r.clone().json().catch(()=>null) as any;
        const code=String(detail?.error?.status||detail?.error?.code||'').toLowerCase();
        const message=String(detail?.error?.message||'').slice(0,300);
        if(['rate_limit_exceeded','too_many_requests'].includes(code)){
          if(attempt===MAX_RETRIES)throw new RetryableAIError(`Gemini rate limit: ${message||'Please retry shortly.'}`);
          await waitForRetry(attempt);
          continue;
        }
        throw new Error(`Gemini quota limit reached: ${message||'Please check your Gemini API quota.'}`);
      }
      break;
    }
